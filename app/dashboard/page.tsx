...
import { LetterInbox, GetUserInboxResponse } from "@/lib/api/types/letter.types";
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const fetchInbox = async () => {
      ...
      try {
        setIsLoading(true);
        const response = await getUserInbox(user.userId);
          ...
          {Object.entries(groupedNewsletters).map(([providerName, letters]) => (
            <div key={providerName} className="space-y-3">
              <div className="flex items-center gap-2">
                ...
                <Badge variant="outline" className="font-normal">{letters.length}</Badge>
              </div>
                        ...
                        <CardHeader className="pb-2">
                          <div className="flex items-start gap-3">
                            ...
                            <div className="flex-1">
                              <h4 className="font-semibold text-lg line-clamp-1 group-hover:text-primary transition-colors duration-300">{letter.title}</h4>
                              <div className="flex items-center mt-1 text-xs text-muted-foreground">
                                <span className="text-primary font-medium">{providerName}</span>
                              </div>
                            </div>
                          </div>
                        </CardHeader>
                          ...
                          <div className="border-t border-border/30 pt-3 flex items-center justify-between">
                            <div className="flex items-center text-sm text-muted-foreground">
                              <div className="flex items-center mr-3">
                                ...
                                <span className="text-xs">{format(new Date(letter.createdDate), "MMM d, yyyy")}</span>
                              </div>
                              <span className="text-xs text-muted-foreground/60">
                                {formatDistance(new Date(letter.createdDate), new Date(), { addSuffix: true })}
                              </span>
                            </div>
